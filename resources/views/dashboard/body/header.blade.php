        ...
        <div class="with-vertical"><!-- ---------------------------------- -->
          ...
          <nav class="navbar navbar-expand-lg p-0">
            ...
            <div class="d-block d-lg-none py-4 py-xl-0">
              <img src="{{asset('/assets/images/logos/farm_logo-removebg-preview.jpeg')}}" alt="Logo" style="width:150px; height: 45px;" />
            </div>
              ...
              <div class="d-flex align-items-center justify-content-between">
                <ul class="navbar-nav d-flex d-xl-none flex-row">
                  ...
                  <li class="nav-item hover-dd dropdown">
                    ...
                    <select class="form-select border fs-3" id="selectCrew" aria-label="Default select example" onchange="fetchCrewDetails(this)" style="background-color: white; margin-right: 10px;">
                      <option selected>Select Client</option>
                    </select>

                    <select class="form-select border fs-3" id="selectMonth" style="background-color: white; margin-right: 10px;">
                    ...
                    <select class="form-select border fs-3" id="selectYear" aria-label="Select Year" style="background-color: white;">
                ...
                <ul class="navbar-nav flex-row ms-auto align-items-center justify-content-center">
                  ...
                  <div class="nav-link">
                    <select class="form-select border fs-3" id="selectCrewboss" aria-label="Default select example" onchange="fetchCrewDetails(this)" style="background-color: white; margin-right: 10px;">
                      <option selected>Select Client</option>
                    </select>
                  ...
                  <li class="nav-item dropdown nav-icon-hover-bg dark rounded-circle">
                    <a class="nav-link" href="javascript:void(0)" id="drop2" data-bs-toggle="dropdown" aria-expanded="false">
                      <img src="{{asset('/assets/images/flag/icon-flag-en.svg')}}" alt="monster-img" width="20px" height="20px" class="rounded-circle object-fit-cover round-20" />
                    </a>
                    <div class="dropdown-menu dropdown-menu-end dropdown-menu-animate-up" aria-labelledby="drop2">
                      <div class="message-body">
                        ...
                        <a href="javascript:void(0)" class="d-flex align-items-center gap-2 py-3 px-4 dropdown-item">
                          <div class="position-relative">
                            <img src="{{asset('/assets/images/flag/icon-flag-cn.svg')}}" alt="monster-img" width="20px" height="20px" class="rounded-circle object-fit-cover round-20" />
                          </div>
                          <p class="mb-0 fs-3">中国人 (Chinese)</p>
                        </a>
                        <a href="javascript:void(0)" class="d-flex align-items-center gap-2 py-3 px-4 dropdown-item">
                          <div class="position-relative">
                            <img src="{{asset('/assets/images/flag/icon-flag-fr.svg')}}" alt="monster-img" width="20px" height="20px" class="rounded-circle object-fit-cover round-20" />
                          </div>
                          <p class="mb-0 fs-3">français (French)</p>
                        </a>
                        <a href="javascript:void(0)" class="d-flex align-items-center gap-2 py-3 px-4 dropdown-item">
                          <div class="position-relative">
                            <img src="{{asset('/assets/images/flag/icon-flag-sa.svg')}}" alt="monster-img" width="20px" height="20px" class="rounded-circle object-fit-cover round-20" />
                          </div>
                  ...
                  <li class="nav-item dropdown">
                    <a class="nav-link" href="javascript:void(0)" id="drop1" aria-expanded="false">
                      <div class="d-flex align-items-center lh-base">
                        <img src="{{asset('/assets/images/profile/user-1.jpg')}}" class="rounded-circle" width="35" height="35" alt="monster-img" />
                      </div>
                    </a>
                    <div class="dropdown-menu content-dd dropdown-menu-end animated flipInY" aria-labelledby="drop1">
                      <div class="profile-dropdown position-relative" data-simplebar>
                        ...
                        <div class="d-flex align-items-center py-9 mx-7 border-bottom">
                          ...
                          <div class="ms-3">
                            <h5 class="mb-1 fs-4">{{ session('user_name') }}</h5>
                            <span class="mb-1 d-block">Designer</span>
                            <p class="mb-0 d-flex align-items-center gap-2">
                              <i class="ti ti-mail fs-4"></i> {{ session('user_email') }}
                            </p>
                          </div>
                        </div>
                        <div class="message-body">
              ...
              <div class="offcanvas-header justify-content-between ps-0 pt-0">
                <div class="brand-logo d-flex align-items-center">
                  <a href="{{ route('user.dashboard') }}" class="text-nowrap logo-img">
                    <img src="{{asset('/assets/images/logos/dark-logo.svg')}}" alt="Logo" class="dark-logo" />
                    <img src="{{asset('/assets/images/logos/light-logo.svg')}}" alt="Logo" class="light-logo" />
                  </a>
                </div>
...
<script>
  $(document).ready(function() {
    ...
    if (clientId.trim() !== "" || month.trim() !== "" || year.trim() !== "") {
        fetchCrewDetails(clientId); 
        $('#selectCrew').attr(clientId); 
    ...
    $.ajax({
        url: "/get-crew-details", 
        type: "GET",
        data: { client_id: clientId },
        success: function(response) {
          const selectCrew = $('#selectCrew, #selectCrewboss');
          selectCrew.empty().append('<option value="">Select a Crew</option>');
          response.data.forEach(user => {
            selectCrew.append(`<option value="${user.id}">${user.name}</option>`);
          });
          if (clientId.trim() !== "") {
                selectCrew.val(clientId);
            }
        },
      ...
      $('#selectMonth, #selectYear').change(function () {
        ...
        let selectedMonth = $('#selectMonth').val() || null;
        let selectedYear = $('#selectYear').val() || null;
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log('Session data saved:', response.message);
            },
            error: function (xhr) {
                console.error('Error saving session data:', xhr.responseText);
            }
        });
    });
  ...
  function fetchCrewDetails(crewIds) {
        var crewId = '';
        const selectedWeek = crewIds.value;
        if(selectedWeek){
          crewId = selectedWeek;
        } else{
          crewId = crewIds;
        ...
        if (crewId) {
            // Send AJAX request
            $.ajax({
                url: '{{ route("user.dashboard") }}', // Replace with your route
                type: 'GET',
                data: { id: crewId },
                success: function(response) {
                  console.log(response.clientusername[0]);
                  if (response.clientusername && response.clientusername.length > 0) {
                      let clientUserHtml = '';
                      let clientUserHtmlJob = ''; 
                      let clientUserHtmlloc = '';
                      let clientUserHtmlexp = '';
                      response.clientusername.forEach(item => {
                       
                          const clientName = item.crewuser ? item.crewuser.name : 'N/A';
                          const jobName = item.jobuser ? item.jobuser.name : 'N/A'; 
                          const RanchName = item.ranchuser ? item.ranchuser.title : 'N/A';
                          const BlockName = item.blockuser ? item.blockuser.title : 'N/A';
                          const AcreName = item.acreuser ? item.acreuser.title : 'N/A';
                          const variety_name = item.variety_name ? item.variety_name : 'N/A';
                          ...
                          clientUserHtml += `
                              <tr>
                                  <td>
                                      <div class="d-flex align-items-center">
                                          <div class="ms-3">
                                              <h6 class="fs-4 fw-normal mb-0">${clientName}</h6>
                                          </div>
                                      </div>
                                  </td>
                              </tr>`;
                              ...
                              <tr>
                                  <td>
                                      <div class="d-flex align-items-center">
                                          <div class="ms-3">
                                              <h6 class="fs-4 fw-normal mb-0">${jobName}</h6>
                                          </div>
                                      </div>
                                  </td>
                              </tr>`;
                              ...
                              <tr>
                                  <td>
                                      <div class="d-flex align-items-center">
                                          <div class="ms-3">
                                              <h6 class="fs-4 fw-normal mb-0">${RanchName}, ${BlockName}, ${AcreName}, ${variety_name}</h6>
                                          </div>
                                      </div>
                                  </td>
                              </tr>`;
                              ...
                              <tr>
                                  <td>
                                      <div class="d-flex align-items-center">
                                          <div class="ms-3">
                                              <h6 class="fs-4 fw-normal mb-0">$ ${item.total_amount}</h6>
                                          </div>
                                      </div>
                                  </td>
                              </tr>`;
                      ...
                      // Append the generated HTML to your desired table or container
                      $('#clientusername').html(clientUserHtml);
                      $('#clientuserjob').html(clientUserHtmlJob);
                      $('#clientuserloc').html(clientUserHtmlloc);
                      $('#clientuserexp').html(clientUserHtmlexp);
                  }
                ...
                error: function(xhr) {
                    console.error("Error fetching crew details.");
                }
            });
        }
    }
